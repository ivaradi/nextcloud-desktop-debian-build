...
versionTagRE = re.compile(r"^v([0-9]+((\.[0-9]+)+))(-(.+))?$")
...
def getCommitVersion(commit):
    ...
    try:
        for line in subprocess.check_output(["git", "show",
            ...
            line = str(line, "utf-8")
            m = re.match(r"set\( *MIRALL_VERSION_([A-Z]+) +([0-9]+) *\)", line)
            if m is not None:
                kind=m.group(1)
                version=m.group(2)
                if kind=="MAJOR":
...
def collectEntries(baseCommit, baseVersion, kind, finalBaseVersion,
    ...
    for line in output.splitlines():
        ...
        revdate = datetime.datetime.fromtimestamp(long(revdate), datetime.UTC).strftime("%Y%m%d.%H%M%S")
        revdate += "." + commit
...
if __name__ == "__main__":
    ...
    finalRevDate = sys.argv[4] if len(sys.argv)>4 and sys.argv[4] else None
    finalBaseVersion = sys.argv[5] if len(sys.argv)>5 and sys.argv[5] else None
    finalCommit = sys.argv[6] if len(sys.argv)>6 and sys.argv[6] else None
